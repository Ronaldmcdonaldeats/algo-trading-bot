#!/usr/bin/env python3
"""
Phase 15: Conservative Realism - Validated improvements on Phase 12 (11.15%)
Target: Beat S&P by 12-15% consistently and realistically
"""
...
class Phase15Conservative:
    """Conservative improvements validated against Phase 12 baseline (11.15%)"""
    ...
    def calculate_features_conservative(self, prices: np.ndarray) -> Dict[str, float]:
        if len(prices) < 200:
            return {}
        
        # Core Phase 12 features (unchanged - proven working)
        ...
        # NEW: Simple trend confirmation (no complex logic)
    ...
    def predict_conservative_signal(self, features: Dict[str, float], prev_signal: int = 0, entry_price: float = 0.0, current_price: float = 0.0) -> Tuple[int, float]:
        if not features:
            return 0, 1.0
        ...
        # Phase 12 base signals (unchanged)
        ...
        # NEW: Slight trend strength modifier (conservative)
        strength_signal = 0.05 if trend_strength > 0.03 else -0.05 if trend_strength < 0.005 else 0.0
        
        # Phase 12 weights with slight enhancement
        vote = (trend_signal * 0.40 + 
                ...
                accel_signal * 0.05 + 
                vol_signal * 0.03 + 
                strength_signal * 0.02)
        
        # Phase 12 position sizing (unchanged)
        if abs(vote) > 0.4:
            position_size = 1.5
        elif abs(vote) > 0.25:
            position_size = 1.3
        ...
        # Phase 12 volatility boost (unchanged)
        if vol_ratio < 0.8:
            position_size *= 1.2
        elif vol_ratio < 0.95:
            position_size *= 1.1
        
        # Phase 12 hysteresis (unchanged)
    ...
    def backtest(self, prices: np.ndarray) -> float:
        ...
        for i in range(200, len(prices)):
            price = prices[i]
            features = self.calculate_features_conservative(prices[:i+1])
            signal, pos_size = self.predict_conservative_signal(features, prev_signal, entry_price, price)
    ...
    def run(self):
        logger.info("Phase 15: Conservative Realism (enhance Phase 12 baseline)")
        ...
        logger.info(f"\n{'='*80}")
        logger.info(f"PHASE 15: CONSERVATIVE REALISM")
        logger.info(f"{'='*80}")
        ...
        logger.info(f"Phase 12 Baseline:      11.15% (10.05% beat)")
        logger.info(f"Improvement:            {(avg - 0.1115)*100:+6.2f}% {'(BETTER)' if avg > 0.1115 else '(WORSE)'}")
        logger.info(f"{'='*80}\n")
        
        out = Path(__file__).parent.parent.parent / 'phase15_results'
        out.mkdir(exist_ok=True)
        pd.DataFrame(results).to_csv(out / 'phase15_conservative_results.csv', index=False)
        logger.info(f"Results saved to {out}")


if __name__ == '__main__':
    Phase15Conservative().run()
