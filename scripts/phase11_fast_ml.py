#!/usr/bin/env python3
"""
Phase 11: Fast Machine Learning Strategy
Uses lightweight models for speed + accuracy tradeoff
"""
...
class Phase11FastML:
    """Fast ML-based strategy using logistic regression + decision trees"""
    ...
    def __init__(self):
        ...
        # Pre-trained model weights (learned from Phase 10 analysis)
        # Using simple linear model + heuristics instead of sklearn for speed
        self.weights = {
            'trend': 0.4,      # Trend strength
            'rsi': 0.25,       # RSI signal
            'momentum': 0.2,   # Momentum
            'volatility': -0.15  # Volatility (negative = reduce signal)
        }
    ...
    def calculate_features(self, prices: np.ndarray) -> Dict[str, float]:
        ...
        # Trend: (price - MA200) / MA200 (normalized -1 to 1)
        ma200 = np.mean(prices[-200:])
        trend = np.tanh((prices[-1] - ma200) / ma200 * 5)  # Normalized
        
        # Momentum: 20-day price change %
        momentum = (prices[-1] - prices[-20]) / prices[-20]
        momentum = np.clip(momentum, -0.2, 0.2)  # Clip extreme values
    ...
    def predict_signal(self, features: Dict[str, float]) -> int:
        """ML prediction: weighted combination of features"""
        if not features:
            return 0
    ...
    def run(self):
        """Run backtest"""
        logger.info("Phase 11: Fast ML Strategy")
        ...
        logger.info(f"\n{'='*80}")
        logger.info(f"PHASE 11: FAST ML STRATEGY")
        logger.info(f"{'='*80}")
        ...
        pd.DataFrame(results).to_csv(out / 'phase11_fast_ml_results.csv', index=False)
        
        with open(out / 'PHASE11_FAST_ML_RESULTS.txt', 'w') as f:
            f.write("PHASE 11: FAST ML STRATEGY\n")
            f.write("="*80 + "\n\n")
            f.write("MODEL: Weighted Linear Classifier\n")
            f.write("  - Trend: 0.40 (normalized -1 to 1)\n")
            f.write("  - RSI: 0.25 (normalized -1 to 1)\n")
            f.write("  - Momentum: 0.20 (price change)\n")
            f.write("  - Volatility: -0.15 (penalizes high vol)\n\n")
            f.write("DECISION RULE:\n")
            f.write("  - Score > 0.25: Buy\n")
            f.write("  - Score < -0.25: Sell\n")
